...
class MultiHeadTemporalAttention(nn.Module):
    """Enhanced multi-head attention with early detection bias"""
    def __init__(self, d_model, num_heads, dropout=0.1, max_seq_len=200, timing_bias_weight=0.1, early_detection_weight=0.2):
        super().__init__()
        ...
        # Early detection bias - learnable parameter that biases attention toward early time steps
        self.early_detection_bias = nn.Parameter(torch.randn(d_model))
        
    def forward(self, x, mask=None):
        batch_size, seq_len = x.size(0), x.size(1)
        
        # Apply linear transformations with early detection bias
        Q = self.q_linear(x + self.early_detection_weight * self.early_detection_bias).view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        K = self.k_linear(x + self.early_detection_weight * self.early_detection_bias).view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        # Add early detection bias to attention scores
        early_bias = self._get_early_detection_bias(seq_len)
        scores = scores + early_bias
    ...
    def _get_early_detection_bias(self, seq_len):
        """Generate early detection bias that favors early time steps"""
        device = self.early_detection_bias.device
        
        # Create bias that decreases with time (favors early steps)
        time_indices = torch.arange(seq_len, device=device, dtype=torch.float)
        early_bias = torch.exp(-time_indices / (seq_len * 0.3))  # Exponential decay
        
        # Expand to match attention score dimensions
        early_bias = early_bias.unsqueeze(0).unsqueeze(0)  # (1, 1, seq_len)
        early_bias = early_bias.expand(self.num_heads, seq_len, seq_len)  # (num_heads, seq_len, seq_len)
        
        return early_bias * self.early_detection_weight
...
class TemporalConv1DBlocks(nn.Module):
    ...
    def __init__(self, input_dim, d_model, dropout=0.1):
        ...
        # Multi-scale temporal convolutions optimized for early detection
        # Calculate channel distribution to ensure exact d_model total
        base_channels = d_model // 5
        extra_channels = d_model % 5
        
        # Distribute channels evenly across the 5 conv blocks
        channel_distribution = [base_channels] * 5
        for i in range(extra_channels):
            channel_distribution[i] += 1
        
        # Verify total channels equals d_model
        total_channels = sum(channel_distribution)
        assert total_channels == d_model, f"Channel distribution error: expected {d_model}, got {total_channels}"
        
        self.conv_blocks = nn.ModuleList([
            # Very early patterns (1-2 hours) - NEW for early detection
            nn.Sequential(
                nn.Conv1d(input_dim, channel_distribution[0], kernel_size=3, padding=1),
                nn.BatchNorm1d(channel_distribution[0]),
                nn.GELU(),
                nn.Dropout(dropout)
            ),
            # Early patterns (2-4 hours) - NEW for early detection
            nn.Sequential(
                nn.Conv1d(input_dim, channel_distribution[1], kernel_size=5, padding=2),
                nn.BatchNorm1d(channel_distribution[1]),
                nn.GELU(),
                nn.Dropout(dropout)
            ),
            # Short-term patterns (4-8 hours) - MODIFIED
            nn.Sequential(
                nn.Conv1d(input_dim, channel_distribution[2], kernel_size=7, padding=3),
                nn.BatchNorm1d(channel_distribution[2]),
                nn.GELU(),
                nn.Dropout(dropout)
            ),
            # Medium-term patterns (8-16 hours) - MODIFIED
            nn.Sequential(
                nn.Conv1d(input_dim, channel_distribution[3], kernel_size=9, padding=4),
                nn.BatchNorm1d(channel_distribution[3]),
                nn.GELU(),
                nn.Dropout(dropout)
            ),
            # Long-term patterns (16-32 hours) - MODIFIED
            nn.Sequential(
                nn.Conv1d(input_dim, channel_distribution[4], kernel_size=11, padding=5),
                nn.BatchNorm1d(channel_distribution[4]),
                nn.GELU(),
                nn.Dropout(dropout)
            )
        ])
...
class EmergencePatternTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.timing_bias_weight = timing_bias_weight
        ...
        # Transformer layers with enhanced attention and early detection
        self.transformer_layers = nn.ModuleList([
            nn.ModuleDict({
                'attention': MultiHeadTemporalAttention(d_model, num_heads, dropout, max_seq_len, timing_bias_weight, early_detection_weight),
    ...
    def forward(self, x, return_attention=False):
        ...
        # Early-focused pooling that emphasizes early time steps
        pooled = self._early_focused_pooling(x, emergence_scores)
    ...
    def _early_focused_pooling(self, x, emergence_scores, early_weight=0.7):
        """Pooling that emphasizes early time steps for better early detection"""
        seq_len = x.size(1)
        
        # Create early-biased weights that decrease with time
        time_indices = torch.arange(seq_len, dtype=torch.float, device=x.device)
        early_weights = torch.exp(-time_indices / (seq_len * 0.3))  # Exponential decay favoring early steps
        
        # Combine with emergence scores
        emergence_weights = emergence_scores.squeeze(-1)  # (batch, seq_len)
        combined_weights = early_weight * early_weights + (1 - early_weight) * emergence_weights
        combined_weights = F.softmax(combined_weights, dim=1)  # (batch, seq_len)
        
        # Apply weighted pooling
        pooled = torch.sum(x * combined_weights.unsqueeze(-1), dim=1)  # (batch, d_model)
        return pooled
...
class SARTransformerLocalTile(nn.Module):
    ...
    """
    def __init__(self, input_dim=5, d_model=128, nhead=4, num_layers=4, dropout=0.1, output_len=12, max_seq_len=500, use_temporal_conv=True, timing_bias_weight=0.1, early_detection_weight=0.2):
        super().__init__()
        ...
        self.transformer = EmergencePatternTransformer(
            ...
            timing_bias_weight=timing_bias_weight,
